...
# -------------------------------
def text_generation_worker(gpt_model_path, gpt_prompt, max_new_tokens, context_length, gpt_device, frame_width, font_scale, font_path_en, font_path_ja):
...
def cli(out_dir, model, labels, size, scale_type, latmask, nxy, splitfine, splitmax, trunc,
        ...
        sg_gpu, font_scale, font_thickness, debug, fullscreen):
    profiler = None
    if debug:
    ...
    try:
        ...
        # バックグラウンドでテキスト生成スレッドを開始
        text_thread = threading.Thread(
            target=text_generation_worker,
            args=(gpt_model, gpt_prompt, max_new_tokens, context_length, gpt_device, size_parsed[1],
